...
typeset -gA ALIAS_TO_EMAIL ALIAS_TO_HOST EMAIL_TO_ALIAS HOST_TO_ALIAS
typeset -ga ALIASES_ORDERED

# Load the identities table: "<alias> <email> <ssh-host>" per line.
_load_identities() {
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    [[ -z "$line" ]] && continue
    fields=( ${=line} )
    local alias="${fields[1]:-}" email="${fields[2]:-}" host="${fields[3]:-}"
